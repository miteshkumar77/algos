...
array<Point, 2> closestPairDivC(const vector<Point>& Px, const vector<int>& Py, int l, int r);
pair<vector<int>, vector<int>> divide(const vector<int>& Py, int mid); 
vector<int> getStrip(const vector<Point>& Px, const vector<int>& Py, double center, double d); 
...
array<Point, 2> closestPairBruteForce(const vector<Point>& points, int l, int r) {
...
array<Point, 2> closestPair(vector<Point> points)
{
    ...
    sort(Py.begin(), Py.end(), [&](int a, int b) -> bool {
...
int main() 
{
    ...
    cout << "ANSWER:   (" << ans[0].x << "," << ans[0].y << "); (" << ans[1].x << "," << ans[1].y << ")" << endl;
    cout << "EXPECTED: (" << expectedans[0].x << "," << expectedans[0].y << "); (" << expectedans[1].x << "," << expectedans[1].y << ")" << endl;
